...
class TransformerDenoisingModel(Module):
    def __init__(self, context_dim=96, tf_layer=2):
        ...
        self.dec_lstm = torch.nn.LSTM(192, 256)
    ...
    def forward(self, x, beta, context, nbrs, mask):
        ...
        ctx_emb = None
        if len(x.shape) == 3:
            ctx_emb = torch.cat([time_emb, context.unsqueeze(1)], dim=-1)  # b,1,3+context embedding
        elif len(x.shape) == 4:
            ctx_emb = torch.cat([time_emb, context.unsqueeze(1)], dim=-1).repeat(1, 3, 1).unsqueeze(2)
        x = self.concat1(ctx_emb, x).contiguous().view(-1, 24, 192)  # b,1,3+context embedding  b,t,2 ->b,t, 192
        # x = self.concat1(context.unsqueeze(1), x).contiguous().view(-1, 24, 192)
