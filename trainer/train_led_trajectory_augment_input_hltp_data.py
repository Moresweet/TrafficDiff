...
class Trainer:
    ...
    def smoothness_loss(self, trajectories):
        # trajectories: tensor of shape (B, 24, 24, 2)
        ...
        for i in range(1, T - 1):
            # 计算当前和下一个点之间的差异
            delta_h_i = delta_h[:, i - 1, :, :]
            delta_h_next = delta_h[:, i, :, :]
            smooth_loss += torch.sum((delta_h_i - delta_h_next) ** 2)

        return smooth_loss / (B * (T - 2) * N)  # 平均损失
