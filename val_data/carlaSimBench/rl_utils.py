        ...
        with tqdm(total=int(num_episodes / 1000), desc='Iteration %d' % i) as pbar:
            for i_episode in range(int(num_episodes / 1000)):
                ...
                while not done and not truncated:
                    ...
                    next_state, reward, done, truncated, info = env.step([action[0][0], action[0][1]])
                    ...
                    next_state = next_state[:11, :].ravel()
                ...
                if (i_episode + 1) % 10 == 0:
                    pbar.set_postfix({'episode': '%d' % (num_episodes / 1000 * i + i_episode + 1),
...
def eval_on_policy_agent(env, agent, num_episodes, return_list, ego_records_list):
    for i in range(1):
...
def train_off_policy_agent(env, agent, num_episodes, replay_buffer, minimal_size, batch_size, return_list,
                           ego_records_list):
    for i in range(1000):
        with tqdm(total=int(num_episodes / 1000), desc='Iteration %d' % i) as pbar:
            for i_episode in range(int(num_episodes / 1000)):
...
def eval_off_policy_agent(env, agent, num_episodes, replay_buffer, minimal_size, batch_size, return_list,
                          ego_records_list):
    for i in range(1):
        with tqdm(total=int(num_episodes / 10), desc='Iteration %d' % i) as pbar:
            for i_episode in range(int(num_episodes / 10)):
...
def state_select(state, env):
    ...
    if np.linalg.norm(sut[0].position - npc_controlled.position) < np.linalg.norm(
            traffic_vehicle[0].position - npc_controlled.position):
        sut_indx = 1  # 因为在next
    else:
        sut_indx = 2
